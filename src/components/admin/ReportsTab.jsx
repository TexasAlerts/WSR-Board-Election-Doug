  ...
  return (
    <div>
      <div className="flex flex-wrap gap-4 items-center mb-6">
        ...
        <button onClick={loadReport} disabled={loading} className="btn-primary flex items-center gap-2 text-sm">
          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <BarChart3 className="w-4 h-4" />}
          Generate Report
        </button>

        {reportData && reportType !== 'engagement' && (
          <button onClick={downloadCSV} className="flex items-center gap-2 text-sm text-navy border border-navy rounded-md px-3 py-2 hover:bg-navy/5">
      ...
      {reportData && reportType === 'engagement' && (
        <EngagementReport data={reportData} />
      )}

      {reportData && reportType === 'polls' && (
        <PollsReport data={reportData} />
      )}

      {reportData && reportType === 'comments' && (
        <CommentsReport data={reportData} />
      )}

      {reportData && reportType === 'ideas' && (
        <IdeasReport data={reportData} />
      )}
    </div>
  );
}
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div className="col-span-2 md:col-span-4 bg-blue-50 rounded-lg p-4 mt-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <div><span className="text-gray-500">Votes:</span> <strong>{data.recentActivity?.votes || 0}</strong></div>
          <div><span className="text-gray-500">Comments:</span> <strong>{data.recentActivity?.comments || 0}</strong></div>
          <div><span className="text-gray-500">Ideas:</span> <strong>{data.recentActivity?.ideas || 0}</strong></div>
          <div><span className="text-gray-500">Sign-ups:</span> <strong>{data.recentActivity?.signups || 0}</strong></div>
        </div>
      </div>
    </div>
  );
      ...
      {data.map(poll => (
        <div key={poll.poll_id} className="bg-white border rounded-lg p-4">
          ...
          <p className="text-sm text-gray-500 mb-3">
            {poll.total_votes} votes &middot; {poll.approved_comments} comments &middot; {poll.poll_type}
          </p>
              ...
              <div key={i} className="flex items-center gap-2">
                <div className="flex-1 bg-gray-100 rounded-full h-6 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${poll.total_votes > 0 ? Math.max((c.votes / poll.total_votes) * 100, 5) : 5}%` }}
                  >
                    <span className="text-white text-xs font-medium">{c.votes}</span>
                  </div>
                </div>
                <span className="text-sm w-40 truncate">{c.text}{c.is_other ? ' (Other)' : ''}</span>
              </div>
            ))}
          </div>
          {poll.other_responses.length > 0 && (
            <div className="mt-3 text-sm">
              <p className="font-medium text-gray-700 mb-1">&quot;Other&quot; responses ({poll.other_responses.length}):</p>
              <ul className="list-disc list-inside text-gray-600">
                {poll.other_responses.map((r, i) => <li key={i}>{r}</li>)}
              </ul>
            </div>
          )}
        </div>
        ...
        <tbody>
          {data.map(c => (
            <tr key={c.id} className="border-b">
              <td className="p-2">{c.name}<br/><span className="text-xs text-gray-400">{c.email}</span></td>
              <td className="p-2 max-w-xs truncate">{c.content}</td>
              <td className="p-2 text-xs">{c.context_type}: {c.context_title}</td>
              <td className="p-2">
                <span className={`px-2 py-0.5 rounded text-xs ${c.status === 'approved' ? 'bg-green-100 text-green-700' : c.status === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  {c.status}
                </span>
              </td>
              <td className="p-2 text-xs">{c.upvotes}↑ {c.downvotes}↓</td>
        ...
        <tbody>
          {data.map(i => (
            <tr key={i.id} className="border-b">
              ...
              <td className="p-2">{i.name}<br/><span className="text-xs text-gray-400">{i.email}</span></td>
              <td className="p-2 text-xs">{i.category}</td>
              <td className="p-2">
                <span className={`px-2 py-0.5 rounded text-xs ${i.status === 'published' ? 'bg-green-100 text-green-700' : i.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'}`}>
                  {i.status}
                </span>
              </td>
              <td className="p-2 text-xs">{i.upvotes}↑ {i.downvotes}↓</td>
